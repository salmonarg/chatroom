...
proxy.get('/sub/:token/shadowrocket', async (c) => {
    ...
    const uris = nodes.results.map(n =>
        n.protocol === 'tuic'
            ...
            : `vless://${row.xray_uuid}@${n.server_ip}:${n.server_port}?security=reality&encryption=none&pbk=${n.public_key}&headerType=none&fp=chrome&type=tcp&flow=xtls-rprx-vision&sni=${n.server_name}&sid=${n.short_id}&packetEncoding=xudp&xudp=1#${encodeURIComponent(n.name)}`
    ).join('\n')
